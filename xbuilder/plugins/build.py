    ...
    def build(self, target_ebuild, target_builder, arch=None):
        ...
        def autocheck(myroot):
            ...
            if stop_on_error and has_errors:
                raise XUtilsError(
                    'Some packages will be installed at an unexpected version. Please fix your profile. For more information, check %s'
                    % self.log_file
                )
        ...
        self.log_fd.flush()
        ret = Popen(['xmerge', '--verbose', '--ignore-default-opts', '--noreplace', 'world'],
        ...
        ret = Popen(['xtc-update', '--automode', '-5'],
                    bufsize=BUILD_LOG_BUFSIZE,
                    stdout=self.log_fd,
                    stderr=self.log_fd,
                    shell=False,
